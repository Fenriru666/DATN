...
class AuthService {
    ...
    if (user != null) {
      try {
        ...
        UserModel newUser = UserModel(
          uid: user.id,
          email: user.email ?? '',
          name: user.userMetadata?['name'] ?? user.email?.split('@')[0] ?? 'Khách hàng',
          roles: [role],
    ...
    try {
      ...
      if (referralCode != null && referralCode.trim().isNotEmpty) {
        final referrerSnapshot = await _supabase
            .from('users')
            .select('id')
            .eq('referral_code', referralCode.trim().toUpperCase()) // Ensure exact match
            .limit(1)
            .maybeSingle();
      ...
      if (referredByUid != null) {
        await _grantReferralRewards(newUserId: user.id, referrerId: referredByUid);
      }
  ...
  String _generateReferralCode(String email, String uid) {
    ...
    String prefix = email.split('@')[0].replaceAll(RegExp(r'[^a-zA-Z0-9]'), '').toUpperCase();
  ...
  Future<void> _grantReferralRewards({required String newUserId, required String referrerId}) async {
    ...
    try {
      final newUserData = await _supabase.from('users').select('wallet_balance').eq('id', newUserId).single();
      final referData = await _supabase.from('users').select('wallet_balance').eq('id', referrerId).single();
      
      final currentNewUserBal = (newUserData['wallet_balance'] ?? 0.0).toDouble();
      final currentReferBal = (referData['wallet_balance'] ?? 0.0).toDouble();

      await _supabase.from('users').update({
        'wallet_balance': currentNewUserBal + 50000.0
      }).eq('id', newUserId);

      await _supabase.from('users').update({
        'wallet_balance': currentReferBal + 50000.0
      }).eq('id', referrerId);
  ...
  Future<void> updateSavedPlace(
      String userId, String label, Map<String, dynamic> locationData) async {
    try {
      // In Supabase we might need RPC to update JSONB deeply, or we fetch & save.
      final userRec = await _supabase.from('users').select('saved_places').eq('id', userId).single();
