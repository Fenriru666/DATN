...
class UserAddressService {
  ...
  Future<void> addAddress(
    ...
    String key =
        name.toLowerCase() == 'home' || name.toLowerCase() == 'nhà riêng'
        ? 'home'
        : (name.toLowerCase() == 'work' || name.toLowerCase() == 'công ty'
              ? 'work'
              : name);
    ...
    final data = await _supabase
        ...
        .eq('id', user.id)
        .single();
    Map<String, dynamic> savedPlaces = data['saved_places'] != null
        ? Map<String, dynamic>.from(data['saved_places'])
        : {};

    // Store as Map structure with lat/long
    savedPlaces[key] = {'address': fullAddress, 'lat': lat, 'lng': lng};
  ...
  Future<void> deleteAddress(String addressId) async {
    ...
    if (data['saved_places'] != null) {
      Map<String, dynamic> savedPlaces = Map<String, dynamic>.from(
        data['saved_places'],
      );
